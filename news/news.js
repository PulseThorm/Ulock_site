...
export async function renderNewsInto(sel){
  ...
  if (!pick){ host.textContent = 'Sin noticias.'; return; }
  host.innerHTML = `
    <section class="panel">
      ...
      <div style="margin-top:8px"><a class="btn" href="/news/${pick.id}">Leer noticia completa</a></div>
    </section>`;
}
...
async function renderArticleByPath(sel){
  ...
  if (!it){ host.innerHTML = `<div class="panel"><h3>Noticia no encontrada</h3><p class="note">Prueba la <a class="btn" href="/news/list">lista</a>.</p></div>`; return; }
...
async function renderList(sel){
  ...
  host.innerHTML = `
    <section class="panel">
      <h2 style="margin:.2rem 0">Noticias</h2>
...
function cardHTML(i){
  ...
  return `
    <article class="news-card">
      ...
      <a class="btn" href="${href}">${i.type==='article'?'Leer artículo':'Abrir'}</a>
    </article>
  `;
}
